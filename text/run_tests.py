...
try:
    TEST_TRIES = int(sys.argv[1]) if len(sys.argv) >= 2 else 10
except ValueError:
...
tests = sorted([file for file in os.listdir(os.path.join(os.getcwd(), 'tests')) if file.startswith('in')])
text_test_ok, text_test_maybe, text_test_fail, leak_test_ok, leak_test_fail, error_test_ok, error_test_fail = 0, 0, 0, 0, 0, 0, 0
os.system('make')
print()
...
for test in tests:
    ...
    for i in range(TEST_TRIES):
        ...
        print(f"TEST NO {test_no}.{i+1} RESULTS:\n========================================")
        if text_result == correct_result:
            print(colors.OKGREEN+f"TEXT OUTPUT TEST NO {test_no}.{i+1} PASSED"+colors.ENDC)
            text_test_ok += 1
        elif text_result == '' or correct_result.find(text_result[:-2]) >= 0:
            print(colors.WARNING+f"TEXT OUTPUT TEST NO {test_no}.{i+1} MAY HAVE BEEN PASSED"+colors.ENDC)
            text_test_maybe += 1
        else:
            print(colors.FAIL+f"TEXT OUTPUT TEST NO {test_no}.{i+1}FAILED"+colors.ENDC)
            text_test_fail += 1
        
        if leaks_index >= 0:
            print(f"{colors.OKGREEN}LEAK TEST NO {test_no}.{i+1} PASSED{colors.ENDC}")
            leak_test_ok += 1
        else:
            print(f"{colors.FAIL}LEAK TEST NO {test_no}.{i+1} FAILED{colors.ENDC}")
        ...
        if errors_index >= 0:
            print(f"{colors.OKGREEN}ERROR TEST NO {test_no}.{i+1} PASSED{colors.ENDC}")
            error_test_ok += 1
        else:
            print(f"{colors.FAIL}ERROR TEST NO {test_no}.{i+1} FAILED{colors.ENDC}")
...
print(f"TEXT: {colors.OKGREEN}certainly passed {text_test_ok}{colors.ENDC}, {colors.WARNING}maybe passed {text_test_maybe}{colors.ENDC}, \
{colors.FAIL}failed {text_test_fail}{colors.ENDC} of {text_test_ok+text_test_maybe+text_test_fail}")
